...
const ICONS = { electronics: '🔌', clothing: '👕', books: '📚' } as const;
...
export default function ProductsPage() {
  ...
  // data-loading is managed imperatively so it never appears in SSR HTML —
  // useEffect only runs on the client, preventing hydration attribute mismatches.
  useEffect(() => {
  ...
  return (
    <main className="container">
      ...
      <div className="test-info">
        ...
        <ul>
          <li>LCP fires late — after the React client component renders the product grid.</li>
          <li>CLS when spinner is swapped for the product grid.</li>
          <li>Filter buttons and cart actions test INP.</li>
        </ul>
      </div>
          ...
          {filtersVisible && (
            <div className="filter-bar">
              {(['all', 'electronics', 'clothing', 'books'] as Category[]).map(cat => (
                ...
                >
                  {cat === 'all' ? 'All' : `${ICONS[cat]} ${cat.charAt(0).toUpperCase() + cat.slice(1)}`}
                </button>
              ))}
            </div>
          )}
          ...
          <div className="products-grid" aria-live="polite">
            {visible.map(p => (
              <div key={p.id} className="product-card">
                <div className="product-thumb">{ICONS[p.category]}</div>
