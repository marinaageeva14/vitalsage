...
export default function Heavy() {
  const [log,       setLog]       = useState<LogEntry[]>([{ id: _logId++, msg: 'Click "Run Computation" or wait for the auto-run.', type: 'info' }]);
  ...
  function addLog(msg: string, type: LogEntry['type'] = 'info') {
    setLog(prev => [{ id: _logId++, msg: `[${new Date().toISOString().slice(11, 23)}] ${msg}`, type }, ...prev.slice(0, 49)]);
  }
  ...
  // Auto-run two long tasks during initial render to inflate TBT
  useEffect(() => {
  ...
  function injectDomNodes() {
    setDomNodes(Array.from({ length: 500 }, (_, i) => i + 1));
  ...
  function thrashLayout() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      document.getElementById('thrash-target')!.style.width = `${w - i}px`;
      total += w;
    }
  ...
  return (
    <main className="container">
      ...
      <div className="test-info">
        ...
        <ul>
          <li><strong>TBT</strong> — Two 200 ms tasks run during component mount.</li>
          <li><strong>INP</strong> — Click "Run Computation" while the page loads to experience delayed response.</li>
          <li><strong>Long Tasks</strong> — each step blocks for ~150 ms.</li>
        </ul>
      </div>
      ...
      <div className="heavy-controls">
        <button
          ...
          onClick={() => runComputation(taskCount)}
          disabled={running}
        >
          {running ? '⏳ Running…' : '▶ Run Computation'}
        </button>
        <button className="btn btn-secondary btn-lg" onClick={() => setLog([])}>
          🗑 Clear Log
        </button>
      ...
      <section className="heavy-section">
        <h2>DOM Stress</h2>
        <p>Creates 500 DOM nodes at once — tests rendering/layout time.</p>
        <button className="btn btn-secondary" onClick={injectDomNodes}>Inject 500 nodes</button>
        {domNodes.length > 0 && (
      ...
      <section className="heavy-section">
        ...
        <button className="btn btn-secondary" onClick={thrashLayout}>
          Thrash Layout (10 reads/writes)
        </button>
